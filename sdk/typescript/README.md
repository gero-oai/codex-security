...
writable and outside the scanned repository. Keep the state directory stable
across linked worktrees: changing it selects separate history, artifacts, and
a dedicated Codex sign-in. `--scan-root` filters recorded artifact paths; it
does not import another state directory's scans.
...
Scan configurations don't store
