...
On macOS and Linux, output and state directories must be private to the current
user (`0700`). Parents and state aliases must have trusted owners and trusted
lexical and resolved parents. Group- or world-writable parents require the sticky
bit; the state directory itself cannot use that exception.

On Windows, state needs a protected ACL granting access only to the current user,
`SYSTEM`, and local Administrators. Its parents must prevent other identities
from replacing it. New state directories receive private ACLs; existing ACLs are
checked without rewriting them. History, sign-in, and publication share these
checks.

An error naming mode `0775` concerns that parent: a private child does not prevent
someone from replacing it. Choose a secure location. Change permissions only on
directories you own and can safely change, never on shared or system parents
merely to suppress an error.

- Set `--output-dir` (SDK `outputDir`) for scan results outside the repository.
- Set `CODEX_SECURITY_STATE_DIR` for private persistent state outside the
  repository. A new location does not move existing history or credentials.
- Set `TMPDIR` (`TEMP` on Windows) for temporary runtime files. A private child
  beneath a trusted, sticky system temporary directory is suitable.

`scan --dry-run` checks the selected output ancestry and configured state without
creating output, changing permissions, or initializing the runtime.
