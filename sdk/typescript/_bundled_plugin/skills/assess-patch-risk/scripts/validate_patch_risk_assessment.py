...
# The validator is bundled inside the npm package. Loading the shared contract
# validator must not leave generated bytecode in that package tree.
...
def semantic_errors(value: dict[str, Any]) -> list[str]:
    ...
    passed_relevant_validation = any(
        item["relevant"] and item["status"] == "passed" for item in validation
    )
    ...
    if len(unknown_ids) != len(known_unknown_ids):
        errors.append("unknown ids must be unique")
    for item in evidence_plan:
        resolved = set(item["resolvesUnknowns"])
        if not resolved <= known_unknown_ids:
            errors.append("evidence plan references an unknown id")
        if any(
    ...
    if (
        ...
        and (
            not value["regressionProtection"]["exactHeadChecksPassed"]
            or not passed_relevant_validation
        )
    ):
        errors.append(
            "strong regression protection requires passing exact-head checks"
        )
    if (
    ...
    if value["regressionLikelihood"]["rating"] == "low" and (
        ...
        errors.append("low regression likelihood requires passing relevant protection")
    if (
        "privileged_boundary" in value["autoMergeExclusions"]
        and value["impact"]["rating"] not in {"high", "critical"}
    ):
        errors.append("a privileged boundary requires high or critical impact")
    ...
    if recommendation == "merge":
        ...
        if material_failure:
            errors.append("merge cannot retain material failure evidence")
        if value["confidence"]["rating"] == "low":
            errors.append("merge requires moderate or high confidence")
        if not value["patch"]["changedFiles"]:
            errors.append("merge requires at least one changed file")
        affected_runtime_roots = set(value["affectedRuntimeRoots"])
        if any(
            item["runtimeRoot"] not in affected_runtime_roots for item in boundaries
        ):
            errors.append(
                "merge requires every material boundary runtime root to be affected"
            )
        if evidence_plan:
    ...
    if recommendation == "hold_for_evidence":
        ...
        elif not critical_unknown_ids <= {
            unknown_id
            for item in evidence_plan
            for unknown_id in item["resolvesUnknowns"]
        }:
            errors.append(
                "hold_for_evidence must address every decision-critical unknown"
            )
        elif not any(
            outcome != "hold_for_evidence"
            for item in evidence_plan
    ...
    if recommendation == "revise" and not revision_evidence:
        errors.append("revise requires affirmative failure evidence")
    if recommendation == "block" and not material_failure:
        errors.append("block requires material safety failure evidence")
