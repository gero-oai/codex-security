...
describe("plugin runtime preparation", () => {
  ...
  test.each(["0.1.37", "0.1.60"])(
    ...
    async (previousVersion) => {
      ...
      const installedNormalizer = join(
        upgraded.installedRoot,
        "scripts",
        "normalize_candidates.mjs",
      );
      expect(await readFile(installedNormalizer)).toEqual(
        await readFile(
          join(PLUGIN_ROOT, "scripts", "normalize_candidates.mjs"),
        ),
      );
      const help = spawnSync("node", [installedNormalizer, "--help"], {
        encoding: "utf8",
      });
