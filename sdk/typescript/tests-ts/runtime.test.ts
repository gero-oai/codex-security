...
describe("plugin runtime preparation", () => {
  ...
  test("replaces a cached predecessor before installing the updated Deep reducer", async () => {
    const root = await temporaryDirectory();
    const previous = await plugin(join(root, "previous"), "0.1.22");
    const previousChunk = join(previous, "mcp", "server.mjs.br.part-000");
    await mkdir(join(previous, "mcp"));
    await writeFile(previousChunk, "previous reducer runtime");
    const home = join(root, "home");
    const marketplace = join(home, "sdk-marketplace");
    await mkdir(home);
    const runCodex: NonNullable<
    ...
    const stagedChunk = join(
      marketplace,
      "plugins",
      "codex-security",
      "mcp",
      "server.mjs.br.part-000",
    );

    expect((await bootstrapPlugin(home, previous, options)).version).toBe(
      "0.1.22",
    );
    expect(await readFile(stagedChunk, "utf8")).toBe(
      "previous reducer runtime",
    );

    const upgraded = await bootstrapPlugin(home, PLUGIN_ROOT, options);
    expect(upgraded.version).toBe(BUNDLED_PLUGIN_VERSION);
    expect(upgraded.version).not.toBe("0.1.22");
    expect(await readFile(stagedChunk)).toEqual(
      await readFile(join(PLUGIN_ROOT, "mcp", "server.mjs.br.part-000")),
    );
  });
