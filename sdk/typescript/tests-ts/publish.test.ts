...
describe("connected Linear publication", () => {
  ...
  test("retains verified issue mappings after model-authored failures if the publication database fails", async () => {
    ...
    await expect(
      ...
      /Could not persist created Linear issues: \[redacted\]\..*publication handoff remains at.*avoid creating duplicate issues/u,
    );
...
describe("publication recovery regressions", () => {
  test("retains completion and rejected-retry evidence without claiming a determinate result", async () => {
    const publication = preparedPublication(2);
    const output = [
      issueEvent(publication.issues[0]!),
      issueEvent(publication.issues[0]!, { status: "failed" }),
      issueEvent(publication.issues[1]!),
    ].join("\n");
    let receipt: PublishScanResult | undefined;
        ...
        dependencies(
          ...
          { stdout: output },
          {
            writeEvents: async (_directory, events) => {
              retained = events;
              return "/synthetic/recovery-events.jsonl";
            },
            writeReceipt: async (result) => {
              receipt = structuredClone(result);
            },
  ...
  test("recovers completed direct issues before honoring cancellation", async () => {
    const publication = preparedPublication(23);
    const controller = new AbortController();
    const settleRemaining = Promise.withResolvers<void>();
    let started = 0;
    let stopped = 0;
    let persisted: string[] = [];
    let receipt: unknown;
      ...
      {},
      {
        linearClient: linearApiClient(publication, {
          create: async (input, signal) => {
            ...
            await settleRemaining.promise;
            stopped += 1;
            throw new Error("Synthetic creation failure after cancellation.");
          },
        }),
        recordPublishedIssues: async (_prepared, issues) => {
          persisted = issues.map(({ issueIdentifier }) => issueIdentifier);
    ...
    await expect(
      publishScanInternal(
        publication.scanDirectory,
        {
          ...
          signal: controller.signal,
          onProgress: ({ type }) => {
            if (type === "handoff_recorded") {
              controller.abort("SIGINT");
              settleRemaining.resolve();
            }
          },
        },
        injected,
