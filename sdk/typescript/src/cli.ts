...
interface SkillRunOptions extends PatchReviewOptions {
  ...
  environment?: NodeJS.ProcessEnv;
  onPatchRiskReport?: (assessment: PatchRiskAssessment) => void;
  ...
  reviewCumulativeCandidate?: PatchReviewCumulativeCandidate;
  patchRiskSnapshot?: PatchReviewWorktreeSnapshot;
  onReviewPublicationCandidate?: (
    candidate: PatchReviewPublicationCandidate,
  ) => void;
  onReviewCumulativeCandidate?: (
    candidate: PatchReviewCumulativeCandidate,
  ) => void;
...
interface PatchReviewCumulativeCandidate extends PatchReviewCandidateDelta {
  base: string;
  head: string;
  diffBytes: Buffer;
...
interface PatchReviewWorktreeSnapshot {
  ...
  cumulativeCandidate?(
    ...
    previous?: PatchReviewCumulativeCandidate,
  ): Promise<PatchReviewCumulativeCandidate>;
...
interface CliDependencies {
  ...
  validatePatchRiskAssessment?: typeof validatePatchRiskAssessment;
      ...
      async run({ format, options }) {
        ...
        try {
          ...
          if (savedFindings) {
            ...
            const patchRisk = patchRun.patchRisk;
          ...
          const patchBase = options.createPr
          ...
          exitCode = await runSkill(
            ...
            dependencies,
            {
              ...
              onPatchRiskReport: (assessment) => {
                patchRisk = assessment;
              },
            },
          );
            ...
            if (options.createPr) {
              ...
              await createPatchPullRequest(
                  ...
                  : `Applies a security fix generated for ${identifier}.`,
                {
                  ...
                  reviewPublicationHead: publicationCandidate?.head,
...
const PATCH_REPORT_BOUNDARY_LINE =
  /^(?<prefix>\s*(?:(?:[-*+]|\d+[.)])\s+)?[*_`]*)(?<label>authorization|token handling)(?<separator>[*_`]*\s*:[*_`]*\s*)(?<evidence>.*)$/iu;
...
const PRIVATE_KEY_BLOCK_START =
  /^\s*[*_`]*-----BEGIN [A-Z ]*PRIVATE KEY(?: BLOCK)?-----[*_`]*\s*$/iu;
const PRIVATE_KEY_BLOCK_END =
  /^\s*[*_`]*-----END [A-Z ]*PRIVATE KEY(?: BLOCK)?-----[*_`]*\s*$/iu;
const PATCH_REPORT_EMPTY_SENSITIVE_FIELD =
  /^\s*(?:(?:[-*+]|\d+[.)])\s+)?[*_`]*(?:(?:api|access|private)[ _-]?key|authorization|auth|token(?:[ _-]?handling)?|secret|credential|signature|password|passwd)[*_`]*\s*[:=][*_`]*\s*$/iu;
...
async function patchReviewObjectDirectoryForGitDirectory(
  gitDirectory: string,
  allowedDirectories: readonly string[],
): Promise<string> {
  ...
  try {
    metadata = await lstat(commondirPath);
  } catch (error) {
  ...
  if (metadata !== undefined) {
    if (!metadata.isFile() || metadata.isSymbolicLink()) {
      throw new CodexSecurityError(
        "Nested Git metadata must use a confined common directory.",
      );
    }
    const bytes = await readFile(commondirPath);
    const contents = bytes.toString("utf8");
    const match = /^([^\r\n]+)\r?\n?$/u.exec(contents);
    if (!Buffer.from(contents, "utf8").equals(bytes) || match === null) {
      throw new CodexSecurityError(
        "Nested Git metadata must use a confined common directory.",
      );
    }
    commonDirectory = await realpath(resolve(gitDirectory, match[1]!));
  }
  if (
    !allowedDirectories.some(
      (directory) => !isOutsidePath(relative(directory, commonDirectory)),
    ) ||
    !(await lstat(commonDirectory)).isDirectory()
  ) {
    throw new CodexSecurityError(
      "Nested Git metadata must use a confined common directory.",
    );
...
async function sealPatchReviewObjects(
  ...
  for (const object of objectList) {
    ...
    const size = match === null ? undefined : Number(match[2]);
    const contentsStart = headerEnd + 1;
    const contentsEnd =
      size === undefined || !Number.isSafeInteger(size)
        ? -1
        : contentsStart + size;
    if (
      match?.[1] !== object ||
      ...
      contentsEnd >= output.length ||
      output[contentsEnd] !== 0x0a
    ) {
      throw new CodexSecurityError(
        "The patch review baseline contains an unreadable Git object.",
      );
    }
    const contents = output.subarray(contentsStart, contentsEnd);
    if (
      (await writePatchReviewBlob(objectDirectory, objectFormat, contents)) !==
      object
    ) {
      throw new CodexSecurityError(
        "The patch review baseline contains an invalid Git object.",
      );
    }
...
function parsePatchReviewTreeEntries(
  ...
  for (const record of splitNulRecords(output)) {
    ...
    const rawPath = separator < 0 ? undefined : record.subarray(separator + 1);
    const path =
      rawPath === undefined ? undefined : decodePatchReviewGitPath(rawPath);
    if (match === null || rawPath === undefined) {
      throw new CodexSecurityError(
        "The selected repository contains an unreadable Git tree entry.",
      );
    }
    ...
    entries.set(path, { path, mode: match[1], object: match[3] });
  }
  return entries;
}

function patchReviewTreeIndexEntries(output: Buffer): Buffer {
  ...
  for (const record of splitNulRecords(output)) {
    ...
    entries.push(
      Buffer.from(`${match[1]} ${match[3]}\t`, "ascii"),
      path,
      Buffer.from([0]),
    );
  }
...
function changedPatchReviewTreeEntryPaths(
  left: ReadonlyMap<string, PatchReviewTreeEntry>,
  right: ReadonlyMap<string, PatchReviewTreeEntry>,
): string[] {
  return [...new Set([...left.keys(), ...right.keys()])].filter(
    (path) => !samePatchReviewTreeEntry(left.get(path), right.get(path)),
  );
}
...
async function snapshotPatchReviewWorktree(
  ...
  const nestedRepositoryState = async (
    ...
    await validatePatchReviewObjectAlternates(
      await patchReviewObjectDirectoryForGitDirectory(
        nested.gitDirectory,
        allowedNestedGitDirectories,
      ),
      allowedNestedGitDirectories,
  ...
  const cumulativeEnvironment = {
    ...objectEnvironment,
    GIT_INDEX_FILE: join(temporaryDirectory, "cumulative-index"),
  };
  ...
  try {
    ...
    const headIndexEntries = patchReviewTreeIndexEntries(headTreeListing);
    if (headIndexEntries.length > 0) {
      await runPatchReviewGit(
        repository,
        ["update-index", "-z", "--index-info"],
        {
          environment: publicationEnvironment,
          input: headIndexEntries,
          signal,
        },
      );
    }
    const publicationBaseTree = await runPatchReviewGit(
      repository,
      ["write-tree"],
    ...
    let baselineRepositoryIndexEntries: Buffer | undefined;
    const repositoryIndexTree = async (): Promise<{
      ...
      if (repositoryIndexSnapshot === 1) {
        await sealPatchReviewIndexObjects(
          repository,
          entries,
          objectDirectory,
          objectFormat,
        ...
        baselineRepositoryIndexEntries = Buffer.from(entries);
      } else if (
        baselineRepositoryIndexEntries === undefined ||
        !entries.equals(baselineRepositoryIndexEntries)
      ) {
        return { entries, tree: "" };
      }
      await runPatchReviewGit(repository, ["read-tree", "--empty"], {
    ...
    const baselinePathsChangedFromHead = changedPatchReviewTreeEntryPaths(
      headEntries,
      baselineEntries,
    );
    ...
    const preexistingPathSet = new Set([
      ...changedPatchReviewTreeEntryPaths(headEntries, indexEntries),
      ...changedPatchReviewTreeEntryPaths(
        headEntries,
    ...
    return {
      ...
      async cumulativeCandidate(candidate, previous) {
        signal?.throwIfAborted();
        if (disposed) {
          throw new CodexSecurityError(
            "The patch review snapshot is no longer available.",
        ...
        if (previous !== undefined && previous.base !== baselineTree) {
          throw new CodexSecurityError(
            "The reviewed patches do not share one cumulative baseline.",
          );
        }
      ...
      async publicationCandidate(candidate, previous) {
        ...
        return {
          ...
          diff: projected.diff,
          diffBytes: projected.diffBytes,
          publicationEntries: projected.entries,
...
async function sealPatchRiskReviewContract(
  ...
  const contents = await Promise.all(
    files.map((path) => readFile(join(plugin, path), "utf8")),
  );
  signal?.throwIfAborted();
  const temporaryRoot = await realpath(tmpdir());
  if (!isOutsidePath(relative(repository, temporaryRoot))) {
  ...
  try {
    ...
    return {
      ...
      rubric: contents[1]!,
      schema: contents[2]!,
      validatorSource: contents[3]!,
      validatorPath: join(root, files[3]),
      async dispose() {
        if (disposed) return;
        disposed = true;
...
async function validatePatchRiskAssessment(
  ...
  const execution = execFile(
    python,
    ["-I", "-S", "-B", options.validatorPath, "--review-envelope", "-"],
    {
  ...
  execution.child.stdin?.on("error", () => {});
  execution.child.stdin?.end(reviewResponse);
  try {
    await execution;
    signal?.throwIfAborted();
    return true;
...
function parsePatchRiskReport(
  ...
  if (start < 0 || end < 0) {
    if (!requireSummary) return { report };
    throw new CodexSecurityError(
...
async function runFindingPatchesWithRiskSnapshot(
    ...
    try {
      status = await runSkill(
        ...
        dependencies,
        {
          ...
          onPatchRiskReport: (assessment) => {
            patchRisk = assessment;
            options.onPatchRiskReport?.(assessment);
          },
          ...
          onReviewCumulativeCandidate: (candidate) => {
            reviewCumulativeCandidate = candidate;
          },
  ...
  if (
    ...
    const beforeFinalReview = await patchRiskSnapshot.candidate();
    for (const stage of finalReviewStages) {
      ...
      const afterFinalReview = await patchRiskSnapshot.candidate();
      if (!samePatchReviewCandidate(beforeFinalReview, afterFinalReview)) {
        finalReviewFailure = `The worktree changed while final combined ${stage} review was running.`;
        break;
      }
      if (review.status === "failed" || review.verdict.status !== "approved") {
        const details =
          review.status === "failed"
            ? review.reason
            : safePatchReport(review.verdict.findings.join("\n")).replaceAll(
                "\n",
                "; ",
              );
        finalReviewFailure = `Final combined ${stage} review did not approve the cumulative patch${details ? `: ${details}` : "."}`;
        break;
      }
    }
    if (finalReviewFailure !== undefined) {
      stderr.write(`${stripPatchControlCharacters(finalReviewFailure)}\n`);
      invalidateVerifiedPatches(finalReviewFailure);
    }
  }
...
interface PatchReviewWorkflowContext {
  ...
  cumulativeCandidate?: PatchReviewCumulativeCandidate;
}
...
async function runIndependentPatchReview(
  ...
  let publicationCandidate: PatchReviewPublicationCandidate | undefined;
  let cumulativeCandidate: PatchReviewCumulativeCandidate | undefined;
  if (stage === "patch-risk-assessment") {
    ...
    if (riskSnapshot !== context.snapshot) {
      const observedCandidate = await riskSnapshot.candidate();
      if (
        candidate.head === undefined ||
        candidate.head !== observedCandidate.head
      ) {
        return {
          status: "failed",
          exitCode: PATCH_REVIEW_EXIT_CODE.failure,
      ...
      if (context.options.createPr === true) {
        if (riskSnapshot.publicationCandidate === undefined) {
          return {
            status: "failed",
            exitCode: PATCH_REVIEW_EXIT_CODE.failure,
            reason:
        ...
        publicationCandidate = await riskSnapshot.publicationCandidate(
          candidate,
          context.options.reviewPublicationCandidate,
        );
        riskCandidate = publicationCandidate;
      } else {
        if (riskSnapshot.cumulativeCandidate === undefined) {
          return {
            status: "failed",
            exitCode: PATCH_REVIEW_EXIT_CODE.failure,
            reason:
              "patch-risk-assessment review cannot compose accepted patch deltas.",
          };
        }
        cumulativeCandidate = await riskSnapshot.cumulativeCandidate(
          candidate,
          context.options.reviewCumulativeCandidate,
        );
        riskCandidate = cumulativeCandidate;
      }
    } else if (context.options.createPr === true) {
  ...
  if (stage === "patch-risk-assessment") {
    ...
    context.cumulativeCandidate = cumulativeCandidate;
  }
  ...
  if (verdict.report !== undefined) {
    const assessment = parsePatchRiskReport(
      safePatchReport(verdict.report),
      context.options.createPr === true && verdict.status === "approved",
    );
    context.options.onPatchRiskReport?.(assessment);
    context.stderr.write(`Patch risk assessment:\n${assessment.report}\n`);
  }
...
async function runPatchReviewWorkflow(
  ...
  while (stageIndex < stages.length) {
    ...
    while (true) {
      ...
      if (review.status === "failed") {
        const failedCandidate = await context.snapshot.candidate();
        context.options.signal?.throwIfAborted();
        const reason = samePatchReviewCandidate(candidate, failedCandidate)
          ? review.reason
          : `${stage} review candidate changed while the terminal outcome was being processed.`;
      ...
      if (
        ...
        const terminalCandidate = await context.snapshot.candidate();
        context.options.signal?.throwIfAborted();
  ...
  if (context.cumulativeCandidate !== undefined) {
    context.options.onReviewCumulativeCandidate?.(context.cumulativeCandidate);
  }
  stdout.write(
      ...
      : review
        ? reviewStage === "patch-risk-assessment"
          ? [
              ...
              `In the report field, put a concise, public-safe Markdown summary between ${PATCH_RISK_SUMMARY_START} and ${PATCH_RISK_SUMMARY_END}. Keep detailed analysis and machine-readable assessment outside those markers; only the summary is included in an automatic pull request.`,
  ...
  if (patchThreshold !== undefined) {
    ...
    try {
      ...
      scanData = {
        ...
        patches,
        ...(patchRun.patchRisk === undefined
          ? {}
          : { patchRisk: { report: patchRun.patchRisk.report } }),
        ...(patchRun.reviewRepository === undefined
          ? {}
          : { patchRepository: patchRun.reviewRepository }),
      };
