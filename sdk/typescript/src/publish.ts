...
export async function publishScanInternal(
  ...
  const linearApiKey = publicationApiKey(options, inheritedEnvironment);
  ...
  if (options.skipExisting) {
    ...
    )(preparedScan, inheritedEnvironment, options.signal);
  ...
  const saveReceipt = async (
    receipt: PublishScanResult,
    receiptEnvironment: NodeJS.ProcessEnv,
  ): Promise<void> => {
    const receiptStateDirectory =
      await prepareCodexSecurityStateDirectory(stateDirectory);
    if (dependencies.writeReceipt !== undefined) {
      await dependencies.writeReceipt(receipt, receiptEnvironment);
    } else {
      await writePublicationReceipt(receipt, receiptStateDirectory);
    }
  };
